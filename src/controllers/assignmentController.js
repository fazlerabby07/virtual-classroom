...
const createAssignment = async (req, res, next) => {
	if (req.user.role === 'teacher' || req.user.role === 'superAdmin') {
		...
		if (req.file) {
			assignmentDetails = {
				...assignmentDetails,
				startDate: moment(assignmentDetails.startDate).format(),
		...
		return res.status(200).json(createResponse(assignment, 'assignment created', false, null));
	} else {
...
const getAllAssignmentByClassroomId = async (req, res, next) => {
	const [assignmentsErr, assignments] = await _p(
		assignmentService.getAllAssignmentByClassroomId(req.params.classroomId),
	);
	if (assignmentsErr) {
...
const updateAssignment = async (req, res, next) => {
	if (req.user.role === 'teacher' || req.user.role === 'superAdmin') {
		...
		const [assignmentErr, assignemt] = await _p(
			assignmentService.updateAssignmentById(assignmentId, updatedAssignment),
		);
...
const assignmentSubmitedByStudent = async (req, res, next) => {
	if (req.user.role === 'student') {
		...
		const [assignmentSubmitedErr, assignmentSubmited] = await _p(
			assignmentService.checkAssignmentSubmitedByStudent(assignmentId, studentId),
		);
		...
		if (req.file) {
			...
			const [assignmentErr, assignemt] = await _p(
				assignmentService.uploadAssignmentByStudent(assignmentId, submition),
			);
			if (assignmentErr) {
				_log(assignmentErr, 'red');
...
const assignemtresultForStudent = async (req, res, next) => {
	if (req.user.role === 'student') {
		...
		const [studentresultErr, studentresult] = await _p(
			assignmentService.getAssignmentResultForStudent(assignmentId, studentId),
		);
		if (studentresultErr) {
